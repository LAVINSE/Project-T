...
namespace ProjectT.Editor.Data
{
    ...
    /// </summary>
    public static class ProjectDataTrialService
    {
        #region 필드
            ...
            out string reason)
        {
            ...
            if (ProjectDataValidation.Validate(stage).Count > 0 || ProjectDataValidation.Validate(selected).Count > 0)
            {
                reason = "기준 스테이지와 선택 데이터의 검사 오류를 먼저 수정하세요.";
                return false;
            }

            var copies = new Dictionary<ScriptableObject, ScriptableObject>();
            ...
            try
            {
                ...
                var battles = trialScene.GetRootGameObjects().SelectMany(root => root.GetComponentsInChildren<BattleSession>(true)).ToArray();
                if (battles.Length != 1)
                {
                ...
                {
                    var definition = serialized.FindProperty("definition");
                    if (definition == null)
                    {
                    ...
                    definition.objectReferenceValue = stageCopy;
            ...
            catch (Exception exception)
            {
                ...
                SWLog.LogError("[ProjectDataTrialService] 시험 준비 실패: " + exception.Message);
                return false;
            }
            finally
            {
        ...
        /// </summary>
        private static ScriptableObject CloneGraph(
            ScriptableObject source,
            Dictionary<ScriptableObject, ScriptableObject> copies)
        {
            if (copies.TryGetValue(source, out ScriptableObject existing))
            {
                return existing;
            }
