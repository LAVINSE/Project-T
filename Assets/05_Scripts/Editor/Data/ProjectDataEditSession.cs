...
namespace ProjectT.Editor.Data
{
    ...
    /// </summary>
    public sealed class ProjectDataEditSession : IDisposable
    {
        #region 필드
        ...
        /// </summary>
        private ProjectDataEditSession(ScriptableObject asset)
        {
            source = asset;
            Reload();
        }
        ...
        /// </summary>
        public static ProjectDataEditSession Create(ScriptableObject asset)
        {
            ...
            {
                SWLog.LogWarning("[ProjectDataEditSession] 준비 실패: 저장된 프로젝트 데이터가 필요합니다.");
                return null;
            }

            return new ProjectDataEditSession(asset);
        }
        ...
        /// </summary>
        public bool TryApply(out List<ProjectDataIssue> issues, out string reason)
        {
            issues = ProjectDataValidation.Validate(draft);
            reason = string.Empty;
            ...
            catch (Exception exception)
            {
                ...
                SWLog.LogError("[ProjectDataEditSession] 적용 실패: " + exception.Message);
                return false;
            }
        }
        ...
        /// </summary>
        public static void CopyValues(ScriptableObject from, ScriptableObject to)
        {
            if (from == null || to == null || from.GetType() != to.GetType())
            {
                SWLog.LogWarning("[ProjectDataEditSession] 복사 실패: 같은 종류의 원본과 대상이 필요합니다.");
                return;
            }
