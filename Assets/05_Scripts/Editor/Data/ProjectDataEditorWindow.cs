...
{
    /// <summary>
    /// Project T 메뉴에서 여는 독립 데이터 제작 창입니다. 미적용 초안은 같은 편집기 세션에서 창을 다시 열어도 복원합니다.
    /// </summary>
    public partial class ProjectDataEditorWindow : EditorWindow
    {
        #region 필드
        private const string StylePath = "Assets/05_Scripts/Editor/Data/ProjectDataEditor.uss";
        private ProjectDataEditSession session;
        ...
        private List<ProjectDataIssue> issues = new List<ProjectDataIssue>();
        ...
        #region 속성
        /// <summary>
        /// 이 창에서 관리하는 데이터 종류입니다. 공통 창의 음수 값은 모든 종류를 허용합니다.
        /// </summary>
        protected virtual int DataKind => -1;

        /// <summary>
        /// 창과 메뉴에 표시하는 종류 이름입니다.
        /// </summary>
        private string KindName => DataKind < 0 ? "데이터" : ProjectDataCatalog.KindNames[DataKind];
        ...
        #region 진입
        /// <summary>
        /// 기존 진입점을 사용하는 호출에서는 클래스 편집기를 엽니다.
        /// </summary>
        ...
        /// </summary>
        public static ProjectDataEditorWindow OpenKind(int kind)
        {
            ProjectDataEditorWindow window;
            switch (kind)
            {
                case 0:
                ...
                case 6:
                    ...
                    break;
                default:
                    SW.Util.SWLog.LogWarning("[ProjectDataEditorWindow] 창 열기 실패: 지원하지 않는 데이터 종류입니다.");
                    return null;
            }
        ...
        {
            titleContent = new GUIContent("Project T · " + KindName);
            minSize = new Vector2(800, 560);
        ...
        private void RefreshCatalog()
        {
            ...
            assets = ProjectDataCatalog.Find(DataKind, search);
        ...
        {
            if (DataKind >= 0 && ProjectDataCatalog.GetKind(asset) != DataKind)
            {
            ...
            var next = ProjectDataEditSession.Create(asset);
            if (next == null)
            {
                return false;
            }
        ...
        private void RebuildDetail()
        {
            ...
            switch (currentPage)
            {
                case "검사":
                    ...
                    break;
                case "미리보기":
                    detail.Add(ProjectDataPreview.Create(session.Draft));
                    break;
                case "참조·사용처":
                    BuildReferences();
                    break;
        ...
        private void RestoreDraft()
        {
            ...
            if (ProjectDataCatalog.GetKind(asset) < 0 || (DataKind >= 0 && ProjectDataCatalog.GetKind(asset) != DataKind))
            {
                return;
            }

            session = ProjectDataEditSession.Create(asset);
