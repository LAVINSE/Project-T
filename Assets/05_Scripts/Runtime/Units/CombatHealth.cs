...
namespace ProjectT.Units
{
    ...
    public sealed class CombatHealth
    {
        ...
        /// 최대 체력으로 새 생명을 시작합니다.
        /// </summary>
        private CombatHealth(float maximum)
        {
            Maximum = maximum;
            Current = maximum;
        }
        ...
        /// </summary>
        public static CombatHealth Create(float maximum)
        {
            if (maximum <= 0f || float.IsNaN(maximum) || float.IsInfinity(maximum))
            {
                SWLog.LogWarning("[CombatHealth] 생성 실패: 최대 체력은 0보다 큰 유한한 수여야 합니다.");
                return null;
            }

            return new CombatHealth(maximum);
        }
