...
namespace ProjectT.Units
{
    ...
    /// </summary>
    [RequireComponent(typeof(AllyDestinationMovement))]
    [UnityEngine.Scripting.APIUpdating.MovedFrom(true, "ProjectT.Defense.Units", "ProjectT.Defense.Runtime", "AllyUnit")]
    public sealed class AllyUnit : SWMonoBehaviour
    {
        #region 필드
        ...
        /// 이동 명령과 현재 이동 상태입니다.
        /// </summary>
        public AllyDestinationMovement Movement { get; private set; }
        ...
        /// </summary>
        public event Action<AllyUnit> AvailabilityChanged;
        ...
        /// </summary>
        public bool Initialize(AllyClassDefinition definition, WalkableBattlefield terrain, Vector2 spawn)
        {
            if (definition == null || !definition.IsValid)
            {
                SWLog.LogWarning("[AllyUnit] 초기화 실패: 클래스 설정이 올바르지 않습니다.");
                return false;
            }

            CombatHealth nextHealth = CombatHealth.Create(definition.MaximumHealth);
            var nextMovement = GetComponent<AllyDestinationMovement>();
        ...
        public void AdvanceRevival(float elapsedSeconds)
        {
            ...
            {
                SWLog.LogWarning("[AllyUnit] 부활 진행 실패: 경과 시간은 0 이상의 유한한 수여야 합니다.");
                return;
            }
