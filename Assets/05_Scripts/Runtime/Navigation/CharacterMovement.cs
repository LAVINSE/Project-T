...
namespace ProjectT.Navigation
{
    ...
    [UnityEngine.Scripting.APIUpdating.MovedFrom(true, "ProjectT.Navigation", "ProjectT.Runtime", "AllyDestinationMovement")]
    public sealed class CharacterMovement : SWMonoBehaviour
    {
        #region 필드
        ...
        public bool Initialize(WalkableBattlefield terrain, Vector2 spawnPosition, float speed)
        {
            ...
            {
                SWLog.LogWarning("[CharacterMovement] 초기화 실패: 전장이 없거나 생성 위치가 통행 가능 영역 밖입니다.");
                return false;
            }

            if (speed <= 0f || float.IsNaN(speed) || float.IsInfinity(speed))
            {
                SWLog.LogWarning("[CharacterMovement] 초기화 실패: 이동속도는 0보다 큰 유한한 수여야 합니다.");
                return false;
            }
        ...
        public void Advance(float elapsedSeconds)
        {
            ...
            {
                SWLog.LogWarning("[CharacterMovement] 이동 실패: 경과 시간은 0 이상의 유한한 수여야 합니다.");
                return;
            }
