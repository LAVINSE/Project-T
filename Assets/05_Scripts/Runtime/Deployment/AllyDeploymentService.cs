...
namespace ProjectT.Deployment
{
    ...
    /// </summary>
    public sealed class AllyDeploymentService
    {
        #region 필드
        ...
        private readonly AllyUnit prefab;
        ...
        /// </summary>
        public static AllyDeploymentService Create(
            BattleDeploymentWallet battleWallet,
            WalkableBattlefield terrain,
            SWPool objectPool,
            AllyUnit allyPrefab,
            Transform unitParent)
        {
            if (battleWallet == null || terrain == null || objectPool == null || allyPrefab == null || unitParent == null)
            {
                SWLog.LogWarning("[AllyDeploymentService] 생성 실패: 지갑·전장·풀·아군 프리팹·배치 부모 참조를 확인해 주세요.");
                return null;
            }

            return new AllyDeploymentService(battleWallet, terrain, objectPool, allyPrefab, unitParent);
        }

        /// <summary>
        /// 지정한 통행 가능 위치에 바로 생성합니다. 생성이 완료된 개체만 결제하며 실패 시 개체를 반납합니다.
        /// </summary>
        public bool TryDeploy(AllyClassDefinition definition, Vector2 destination, out AllyUnit unit, out string reason)
        {
            unit = null;
            reason = null;
            if (purchasing)
            ...
            try
            {
                instance = pool.Spawn(prefab.gameObject, destination, Quaternion.identity, parent);
                if (instance == null)
                {
                ...
                AllyUnit created = instance.GetComponent<AllyUnit>();
