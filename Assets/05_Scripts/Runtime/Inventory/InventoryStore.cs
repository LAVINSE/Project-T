...
{
    /// <summary>
    /// 영구 아이템을 종류별로 합산하고 저장 성공 후에만 획득·파괴 결과를 반영합니다. 슬롯 수 제한은 없습니다.
    /// </summary>
    public sealed class InventoryStore
    {
        ...
        /// <summary>
        /// 저장이 완료된 아이템 목록입니다. 최초 획득 순서를 유지하며 변경 시 새 목록으로 교체합니다.
        /// </summary>
        ...
        {
            var items = new List<InventoryStack>(data.Quantities.Count);
            foreach (InventoryQuantity quantity in data.Quantities)
            {
                definitions.TryGetValue(quantity.Identifier, out ItemData definition);
                items.Add(new InventoryStack(quantity, definition));
            }
